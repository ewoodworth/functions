...
def num_space(sentance_with_spaces):
    for i in sentance_with_spaces:
        if i != " ":
            sentance_with_spaces = sentance_with_spaces.replace(i, "")
    return len(sentance_with_spaces)
#alternately
# def num_spaces(sentance_with_spaces):
#     count = 0
#     for i in sentance_with_spaces:
#         if i == " ":
#             count = count + 1
...
def total_meal_price(meal_price, tip_percentage= .15):
    return (meal_price) * (1 + tip_percentage)
...
sign_and_parity(237)
#The first if statement tests number for it's size relative to zero, and links sign to the strings 'Zero' (if equivalent), "Negative' if number is less than zero and 'Positive' if the number is greater than zero. In this case, it returns 'Positive' since 237 is greater than zero
#The second if statement tests the number for a remainder when divided by 2. This will erturn zero as an even number (which makes math work better, but can be argued against. Ask your friendly neighborhood mathematician, but bring snacks as the answer could take a while.) If this test fails, the if-statement returns 'Odd', as in this case where 237 % 2 does not equal zero.
print sign_and_parity(237)
